...
class LMNN_PP:
  ...
  def process_input_old(self, X, pp_train, X_val=None, pp_val=None):
    self.X_train = X
  ...
  def report(self):
    ...
    print self.n_iter_total, 'ijl: %d\t%.2f, %.4f, p_target:%.4f,%.4f,%.4f'\
    %(len(self.ijl), self.loss, self.M.mean(), p_target_train, p_target_val, p_target_test)
    self.stats.append((len(self.ijl), self.loss, p_target_train, p_target_val, p_target_test))
  ...
  def neigh_pp_mean(self):
    ...
    _, knn = NearestNeighbors(self.k, algorithm='brute', metric='mahalanobis', VI=self.M).fit(self.X_train).kneighbors(self.X_test)
...
def train_lmnn():
  k = 11;mu=0.5;c=1;v=0.05
  X, y = loadPima()
  folds = [(tr, te) for tr, te in StratifiedKFold(y, 5)]
  max_iter = 500
  rr = []
...
if __name__ == '__main__':
  rr = train_lmnn()
  pickle.dump(rr, open('Ionok11mu.5c1v.05max500.pickle', 'wb'))
